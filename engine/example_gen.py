...
def _fallback_examples(catalog: Dict[str, List[str]], stats: Dict[str, Any]) -> List[str]:
    ...
    if customers and products:
        examples.append(f"Add customer {customers[0]}_New; demand 8000 of {products[0]}; lead time 9")
    if warehouses:
        examples.append(f"Add warehouse {warehouses[0]}_Annex at {warehouses[0].split('_')[0]}; Maximum Capacity 30000; force open")
    if modes and warehouses and customers and products:
        examples.append(f"Delete {modes[0]} lane {warehouses[0]} -> {customers[0]} for {products[0]} in 2023")
    return examples
